...
const HeroSection = ({ popularMovies }: Props) => {
  ...
  return (     
    <section className="w-full">
      <Carousel className="w-full" responsive={carouselResponsive} itemClass={"carousel-item-center"} autoPlay autoPlaySpeed={5000}>
        {popularMovies?.slice(0, 5).map((movie) => (
          <article className="relative h-[450px] md:h-[600px] w-full flex flex-col justify-end items-center" key={movie.id}>
            <Image
            ...
            <div className="relative flex flex-col justify-end items-center z-20 mb-10 w-full page-padding-x">
              ...
              <p className="text-gray-300 mt-2 text-center">{getMovieGenreFromIds(movie.genre_ids)}</p>
